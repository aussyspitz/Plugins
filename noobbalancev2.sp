...
new Float:autobalance_check_rate = 5.0;	//check if teams are balanced every this many seconds
...
{
	new diff = ((GetTeamClientCount(TEAM_RED) - (GetRealClientsInGameByTeam(TEAM_RED)) - (GetTeamClientCount(TEAM_BLUE) - (GetRealClientsInGameByTeam(TEAM_BLUE)))));
		if ((diff > 1) || (diff < -1))
		return false;
	else
		return true;
