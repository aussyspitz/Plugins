...
new Float:autobalance_check_rate = 10.0;	//check if teams are balanced every this many seconds
...
{
	new diff = GetRealClientsInGameByTeam(TEAM_RED) - GetRealClientsInGameByTeam(TEAM_BLUE);
	if ((diff > 1) || (diff < -1))
		return false;
	else
		return true;
...
/*	Force-balances teams, making sure they have the same player count.
	...
	*/
	...
	new delta = GetRealClientsInGameByTeam(TEAM_RED) - GetRealClientsInGameByTeam(TEAM_BLUE);
...
GetRealClientsInGameByTeam(team)
{
	new i, num_clients = 0;
	for(i = 1; i <= MaxClients; i++) {
